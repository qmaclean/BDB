...
#tracking<-tracking %>% filter(frameId > 45)
...
bdb_get_vars_punt_key_event <- function(df_tracking){
...
tr<-bdb_get_vars_punt_key_event(tracking)
...
write.csv(pred,"gunner_key_event_tackle_prob.csv")

write.csv(tr,"key_event_total_tracking.csv")
