...
##tracking %>%
##  group_by(nflId) %>%
##  mutate(TackleOpp = ifelse(TackleOpp == "Y",1,0)) %>%
##  summarize(TackleOpp = sum(TackleOpp)) %>%
##  arrange(desc(TackleOpp))

#lookup sample plays to animate or put a model on
#sam<-tracking %>% filter(nflId == "48784" & TackleOpp == "Y")
...
bdb_get_vars_punt_sample <- function(df_tracking){
...
sample<-tracking %>% filter(gameId=="2020121000",playId=="2876")

#tracking_vars<-bdb_get_vars_punt(tracking)
tr<-bdb_get_vars_punt_sample(sample)
...
write.csv(sample,"sample_tackle_prob.csv")
